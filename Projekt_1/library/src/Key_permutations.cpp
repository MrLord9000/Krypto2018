#include "Key.hpp"

uint64_t Key::permutedChoice_I_64_56()
{
    bitset<56> permutation;
    bitset<64> original = static_cast<bitset<64>>(baseKey);

    const unsigned short permPositions[56] = {57, 49, 41, 33, 25, 17,  9,  1, 58, 50, 42, 34, 26, 18,
                                              10,  2, 59, 51, 43, 35, 27, 19, 11,  3, 60, 52, 44, 36,
                                              63, 55, 47, 39, 31, 23, 15,  7, 62, 54, 46, 38, 30, 22,
                                              14,  6, 61, 53, 45, 37, 29, 21, 13,  5, 28, 20, 12, 4};

    for(int i = 0, j = 55; i < 56; i++, j--)
    {
        permutation[j] = original[ convBE(permPositions[i], 64) ];
    }
    return permutation.to_ullong();
}

uint64_t Key::permutedChoice_II_28_28_48()
{
    bitset<48> permutation;
    bitset<56> original = static_cast<bitset<56>>(keySides.left);
    original <<= 28;
    original |= static_cast<bitset<56>>(keySides.right);

    const unsigned short permPositions[48] = {14, 17, 11, 24,  1,  5,  3, 28, 15,  6, 21, 10,
                                              23, 19, 12,  4, 26,  8, 16,  7, 27, 20, 13,  2,
                                              41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48,
                                              44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};

    for(int i = 0; i < 48; i++)
    {
        permutation[i] = original[ permPositions[i] ];
    }
    return permutation.to_ullong();                                  
}